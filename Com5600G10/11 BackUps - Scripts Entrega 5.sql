...
-- 9) Back-Up completo
CREATE OR ALTER PROCEDURE DBA.BackupCompleto
AS
BEGIN
    BEGIN TRY
        BACKUP DATABASE Com5600G10 
        TO DISK = 'C:\Users\user\Desktop\Com5600G10\BackUps\Com5600G10_backup.bak'
        WITH FORMAT, 
             MEDIANAME = 'BackupMedia',
             NAME = 'Full Backup of Com5600G10',
             STATS = 10;

        PRINT 'Backup completo realizado exitosamente en C:\Users\user\Desktop\Com5600G10\BackUps.';
    END TRY
    BEGIN CATCH
        PRINT 'Error al realizar el backup completo.';
        PRINT ERROR_MESSAGE();
    END CATCH
END;
GO
...
-- 10) Automatizacion de Back-Up completo
IF NOT EXISTS (SELECT 1 FROM msdb.dbo.sysjobs WHERE name = N'Ejecucion_Mensual_BackupCompleto')
BEGIN
    -- Crear el trabajo
    EXEC msdb.dbo.sp_add_job
        @job_name = N'Ejecucion_Mensual_BackupCompleto',
        @description = N'Ejecución automática mensual de backup completo de la base de datos Com5600G10';

    -- Agregar un paso al trabajo para ejecutar el procedimiento
    EXEC msdb.dbo.sp_add_jobstep
        @job_name = N'Ejecucion_Mensual_BackupCompleto',
        @step_name = N'Ejecutar BackupCompleto',
        @subsystem = N'TSQL',
        @command = N'EXEC DBA.BackupCompleto;',
        @database_name = N'Com5600G10',
        @retry_attempts = 3,  -- Intentar hasta 3 veces si falla
        @retry_interval = 5;  -- Esperar 5 minutos entre intentos

    -- Crear un horario para ejecutar el trabajo el primer día de cada mes a las 3:00 AM
    EXEC msdb.dbo.sp_add_schedule
        @schedule_name = N'Ejecucion_Mensual_3AM_BackupCompleto',
        @freq_type = 8, -- Mensual
        @freq_interval = 1, -- El primer día de cada mes
        @freq_recurrence_factor = 1, -- Necesario para frecuencia mensual
        @active_start_time = 30000; -- 3:00 AM

    -- Adjuntar el horario al trabajo
    EXEC msdb.dbo.sp_attach_schedule
        @job_name = N'Ejecucion_Mensual_BackupCompleto',
        @schedule_name = N'Ejecucion_Mensual_3AM_BackupCompleto';
    ...
    PRINT 'El trabajo "Ejecucion_Mensual_BackupCompleto" ha sido creado exitosamente.';
END
ELSE
    PRINT 'El trabajo "Ejecucion_Mensual_BackupCompleto" ya existe.';
GO


-- 11) Back-Up diferencial
CREATE OR ALTER PROCEDURE DBA.BackupDiferencialSemanal
AS
BEGIN
    -- Realizar un backup diferencial de toda la base de datos Com5600G10
    BACKUP DATABASE Com5600G10
    TO DISK = 'C:\Users\user\Desktop\Com5600G10\Backups\Com5600G10_Diferencial.bak'
    WITH DIFFERENTIAL,  -- Especifica que es un backup diferencial
         NOFORMAT,      -- No formatear el dispositivo de backup
         INIT;          -- Inicializa el backup (si es el primer diferencial)
...
-- 12) Automatizacion de Back-Up diferencial
IF NOT EXISTS (SELECT 1 FROM msdb.dbo.sysjobs WHERE name = N'Ejecucion_Semanal_BackupDiferencial')
BEGIN
    BEGIN TRY
        -- Crear el trabajo
        EXEC msdb.dbo.sp_add_job
            @job_name = N'Ejecucion_Semanal_BackupDiferencial',
            @description = N'Ejecución automática semanal de backup diferencial de la base de datos Com5600G10';

        -- Agregar un paso al trabajo para ejecutar el procedimiento
        EXEC msdb.dbo.sp_add_jobstep
            @job_name = N'Ejecucion_Semanal_BackupDiferencial',
            @step_name = N'Ejecutar BackupDiferencialSemanal',
            @subsystem = N'TSQL',
            @command = N'EXEC DBA.BackupDiferencialSemanal;',
            @database_name = N'Com5600G10',
            @retry_attempts = 3,  -- Intentar hasta 3 veces si falla
            @retry_interval = 5;  -- Esperar 5 minutos entre intentos

        -- Crear un horario para ejecutar el trabajo semanalmente a las 3:00 AM
        EXEC msdb.dbo.sp_add_schedule
            @schedule_name = N'Ejecucion_Semanal_3AM_BackupDiferencial',
            @freq_type = 8, -- Semanal
            @freq_interval = 1, -- Cada semana
            @freq_recurrence_factor = 1, -- Necesario para frecuencia semanal
            @active_start_time = 30000; -- 3:00 AM
        ...
        PRINT 'El trabajo "Ejecucion_Semanal_BackupDiferencial" ha sido creado exitosamente.';
    END TRY
    BEGIN CATCH
        PRINT 'Error al crear el trabajo de backup diferencial semanal.';
        PRINT ERROR_MESSAGE();
    END CATCH
END
ELSE
    PRINT 'El trabajo "Ejecucion_Semanal_BackupDiferencial" ya existe.';
GO
