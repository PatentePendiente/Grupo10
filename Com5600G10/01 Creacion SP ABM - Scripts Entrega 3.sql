...
BEGIN
    ...
    SET fechaBorrado = GETDATE() -- Fecha de borrado l�gico
    WHERE nombreProd = @nombreProd; -- Filtrar por el nombre del producto

    -- Verificaci�n de si se realiz� el borrado l�gico
    IF @@ROWCOUNT = 0
        PRINT ('No se encontr� el producto que se desea borrar')
		--RAISEERROR('No se encontr� el producto que se desea borrar', 16, 1);
    ELSE
        -- Si se actualiz� correctamente, confirmar el borrado
...
AS
BEGIN
    -- Realizar el borrado l�gico (actualizar la fecha de borrado)
    UPDATE HR.Empleado
    SET fechaBorrado = GETDATE() -- Fecha de borrado l�gico
    ...
    -- Verificaci�n de si se realiz� el borrado l�gico
    IF @@ROWCOUNT = 0
        -- Si no se actualiz� ning�n registro, lanzar un error
        --RAISEERROR('No se encontr� el empleado con el legajo proporcionado.', 16, 1);
...
BEGIN
    ...
    BEGIN
        ...
        SET @nroFactura = SCOPE_IDENTITY(); -- Obtener el nro de factura reci�n creado
    END
